<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between bg-gray-900 text-white px-4 py-2 rounded-md shadow">
            <h2 class="text-lg font-semibold">Editar Novedad: {{ $novedad->tipo_novedad }}</h2>
                ...
                <form action="{{ route('novedades.update', $novedad->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="vehiculo_id" class="w-full border-gray-300 rounded-md dark:bg-gray-700 dark:text-gray-200">
                                ...
                                @foreach (\App\Models\Vehiculo::all() as $vehiculo)
                                    <option value="{{ $vehiculo->id }}" {{ $novedad->vehiculo_id == $vehiculo->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" name="tipo_novedad" value="{{ old('tipo_novedad', $novedad->tipo_novedad) }}" class="w-full border-gray-300 rounded-md dark:bg-gray-700 dark:text-gray-200" placeholder="Ej: Accidente, Mantenimiento">
                        </div>

                        <div>
                            <label class="font-bold text-gray-700 dark:text-gray-200">Aplica:</label>
                            <input type="text" name="aplica" value="{{ old('aplica', $novedad->aplica) }}" class="w-full border-gray-300 rounded-md dark:bg-gray-700 dark:text-gray-200" placeholder="Ej: Sí, No">
                        </div>

                        <div>
                            <label class="font-bold text-gray-700 dark:text-gray-200">Observaciones:</label>
                            <input type="text" name="observaciones" value="{{ old('observaciones', $novedad->observaciones) }}" class="w-full border-gray-300 rounded-md dark:bg-gray-700 dark:text-gray-200" placeholder="Opcional">
                        </div>
